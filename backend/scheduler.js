...
const { getAllMembers, updateAllMembers } = require('./dbSupabase');
const whatsapp = require('./whatsapp');

const TARGET_GROUP_NAME = process.env.TARGET_GROUP_NAME || 'Payment reminder';
...
cron.schedule('0 0 1 * *', async () => {
    ...
    try {
        await updateAllMembers({ payment_status: 'Not Paid', month: newMonth });
        console.log('Successfully reset all payment statuses in Supabase.');
    } catch (err) {
